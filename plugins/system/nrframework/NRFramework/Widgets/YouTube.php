...
class YouTube extends Video
{
	...
	protected $video_widget_options = [
		 ...
		 * - white
		 * 
		 * Setting it to white will disable the "modestbranding" option
		 */
		'color' => 'red',
	...
	protected function prepare()
	{
		...
		else if ($this->options['coverImageType'] === 'custom' && !empty($this->options['coverImage']))
		{
			...
			$this->options['coverImage'] = 'url("' . \JURI::base() . reset($coverImage) . '")';
		}
		...
		$atts = [
			...
			'data-video-branding="' . var_export($this->options['modestbranding'], true) . '"',
		];
	...
	{
		\JHtml::script('plg_system_nrframework/widgets/video/youtube.js', ['relative' => true, 'version' => 'auto']);
	}
}