...
<tr class="category<?php echo $this->escape($category->class_sfx) . $txt; ?>">
  ...
  <td class="py-2 px-3">
    ...
    <div class="started">
      <span  class="ktopic-category d-block mb-1 fs-sm text-secondary"> <?php echo Text::sprintf('COM_KUNENA_CATEGORY_X', $this->getCategoryLink($topic->getCategory(), null, $topic->getCategory()->description, KunenaTemplate::getInstance()->tooltips())) ?></span>
    ...
    <div class="d-flex align-items-center">
      <?php if ($config->avatarOnCategory) : ?>
        ...
        <?php else : ?>
          <div class="col-md-12">
        <?php endif; ?>
          <span class="d-block mb-1 lastpostlink"><?php echo $this->getTopicLink($topic, 'last', Text::_('COM_KUNENA_GEN_LAST_POST'), null, KunenaTemplate::getInstance()->tooltips(), $category, false, true); ?>
          ...
          <span class="datepost fs-sm text-secondary"><?php echo $topic->getLastPostTime()->toKunena('config_postDateFormat'); ?></span>
          </div>
        </div>
    </div>
  </td>
